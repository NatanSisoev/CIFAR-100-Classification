...
\begin{abstract}    
    ...
    Aquest treball descriu l'aplicació de xarxes neuronals convolucionals per a la classificació d'imatges del dataset \textbf{CIFAR-100}. S'ha implementat un model \textit{SmallResNet} amb blocs residuals lleugers per optimitzar el compromís entre rendiment i nombre de paràmetres. S'han aplicat tècniques d'augmentació de dades i regularització, així com un scheduler de tipus Cosine Annealing per al \textit{learning rate}. Els resultats mostren un Top-1 Accuracy del TODO\% amb només TODO paràmetres, evidenciant l'eficàcia de l'arquitectura i les tècniques de preprocesament utilitzades.
\end{abstract}

%----------------------------------------------------------

%% TODO: Set appropriate keywords for your report.
\keywords{Xarxes neuronals convolucionals, Residual blocks, Classificació d'imatges, Aprenentatge profund, ResNet, CIFAR-100}
...
\section{Introducció}
\taustart{L'}exercici planteja la classificació d'imatges utilitzant el dataset real \textbf{CIFAR-100}. L'objectiu és entrenar una xarxa neuronal convolucional eficient que maximitzi l'\textit{accuracy} amb el menor nombre de paràmetres possible.

El motiu és aplicar els coneixements adquirits als temes 1--4 de l'assignatura sobre arquitectures CNN i tècniques de regularització.

\section{Metodologia}
\subsection{Preprocessament i Augmentació}

Per la part de preprocessament s'ha utilitzat un \textit{TrivialAugmentWide}~\cite{muller2021trivialaugment} que aplica transformacions aleatòries d'intensitat similar a les dades d'entrenament.

Els conjunts d'entrenament i test s'han normalitzat segons la mitjana i desviació estàndard de CIFAR-100. S'han aplicat tècniques d'augmentació per reduir overfitting: \textit{RandomCrop}, \textit{RandomHorizontalFlip}, \textit{RandomRotation}, i \textit{ColorJitter}.

\subsection{Arquitectura del model}
S'ha implementat un \textbf{SmallResNet} amb blocs residuals bàsics de dues convolucions \(3\times3\). La xarxa consta de:

\begin{itemize}
    \item \textbf{Stem:} Conv2d(3, 8) + BatchNorm + ReLU
    \item \textbf{Stage1:} 2 blocs residuals, 16 canals
    \item \textbf{Stage2:} 2 blocs residuals, 64 canals, stride 2
    \item \textbf{Stage3:} 1 bloc residual, 128 canals, stride 2
    \item \textbf{Head:} AdaptiveAvgPool2d + Dropout(0.3) + Linear(128, 100)
\end{itemize}

El nombre total de paràmetres entrenables és $\approx$ TODO.
...
\begin{itemize}
    \item \textbf{Funció de pèrdua:} CrossEntropy amb \textit{label smoothing} 0.1.
    \item \textbf{Optimitzador:} SGD amb learning rate 0.1, momentum 0.9, decaïment de pesos 5e-4, Nesterov activat.
    \item \textbf{Scheduler:} CosineAnnealingLR amb $T_\text{max} = 200$ èpoques.
    \item \textbf{Batch size:} 128.
    \item \textbf{Device:} GPU si està disponible, CPU altrament.
\end{itemize}

\section{Resultats}
Els resultats s'han monitoritzat amb pèrdua i \textit{Top-1 Accuracy} per època. El gràfic final és el següent:



\section{Discussió / Anàlisi}
El model SmallResNet mostra un bon compromís entre complexitat i rendiment. L'augmentació i regularització han contribuït a reduir l'overfitting. L'ús de blocs residuals facilita el flux del gradient, permetent un entrenament estable malgrat la profunditat moderada de la xarxa. El scheduler de tipus cosí ha afavorit una convergència més suau i estable del \textit{learning rate}.

\section{Conclusions}
\begin{itemize}
    \item SmallResNet permet assolir un 61\% de Top-1 Accuracy amb només 33K paràmetres.
    \item L'augmentació i la normalització són essencials per treballar amb conjunts de dades petits i complexos.
    \item Els blocs residuals milloren la capacitat d'aprenentatge i estabilitat durant entrenament llarg.
\end{itemize}
...
\section{Figures and tables} \label{sec:table}
	...
	\begin{figure}[H]
		\centering
		\fbox{\rule{0.75\linewidth}{0.3\linewidth}}
